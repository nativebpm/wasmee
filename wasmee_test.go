...
import (
	...
	"gitlab.com/nativebpm/wasmee/client"
)
...
func (s *testInMemoryStore) SaveMetadata(ctx context.Context, meta *olme.InstanceMeta) (bool, error) {
	...
	if exists {
		println("SaveMetadata: exists=true, prev.Version=", prev.Version, "meta.Version=", meta.Version)
	} else {
		println("SaveMetadata: exists=false, meta.Version=", meta.Version)
	}
...
func TestWasmRunnerExecution(t *testing.T) {
	...
	wasmBytes, err := os.ReadFile("../connectors/wasman/testdata/dirty_page_oplog.wasm")
	if err != nil {
	...
	runner, err := client.NewRunner(ctx, wasmBytes, "localhost:50051")
	if err != nil {
		t.Fatalf("failed to create runner: %v", err)
	}
	defer runner.Close(ctx)

	session := client.NewSession(instanceID, state)
...
func TestWasmRunnerSimulatedCrashRecovery(t *testing.T) {
	...
	session2 := client.NewSession(instanceID, state2)
