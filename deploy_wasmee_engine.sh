...
if [ "$ENV" = "test" ]; then
    SERVICE_NAME="wasmee-engine-test"
else
    SERVICE_NAME="wasmee-engine"
fi
...
BUILD_ID=$(gcloud builds submit --gcs-source-staging-dir="gs://nativebpm-build-sources-133825711702/source" --tag "$IMAGE_TAG" . --async --format="value(id)")
...
echo "WASMEE Engine successfully deployed to $SERVICE_NAME!"
